...
export class PageSubmit extends LitElement {
  ...
  postBusiness(business) {
    ...
    fetch(`${protocol}//${currentEnv}/wp-json/wp/v2/business`, {
      method: 'POST',
      headers: new Headers({
  ...
  async getJWT() {
    const token = await fetch(`${protocol}//${currentEnv}/wp-json/jwt-auth/v1/token`, {
      method: 'POST',
  ...
  async getCategories() {
    ...
    const categories = await fetch(`${protocol}//${currentEnv}/wp-json/wp/v2/categories`)
      .then(response => response.json());
